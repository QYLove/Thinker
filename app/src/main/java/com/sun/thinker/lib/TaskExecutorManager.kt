...
class TaskExecutorManager private constructor() {
    ...
    //获得cpu密集型线程池，因为占据cpu的时间片过多的话会影响性能，
    //所以这里控制了最大并发，防止主线程的时间片减少
    //CPU密集型任务的线程池
    val cpuThreadPoolExecutor: ThreadPoolExecutor

    //获得IO密集型线程池，有好多任务其实占用的cpu time非常少，
    //所以使用缓存线程池，基本上来着不拒
    //IO密集型任务的线程池
    val mIOThreadPoolExecutor: ExecutorService

    //线程池队列
    private val mPoolWorkQueue: BlockingQueue<Runnable> = LinkedBlockingQueue()

    //这个是为了保障任务超出BlockingQueue的最大值，
    //且线程池中的线程数已经达到MAXIMUM_POOL_SIZE时候，
    //还有任务到来会采取任务拒绝策略，
    //这里定义的策略就是在开一个缓存线程池去执行。
    //当然BlockingQueue默认的最大值是int_max，
    //所以理论上这里是用不到的
    ...
    companion object {
        ...
        @JvmStatic
        val mInstance:TaskExecutorManager by lazy(mode = LazyThreadSafetyMode.SYNCHRONIZED) {
            TaskExecutorManager()
        }
    }
